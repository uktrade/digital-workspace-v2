...
def search_category(context, *, category,tab_name=None, limit=None, show_heading=False):
...
def autocomplete(request, query,all_results=False):
    ...
    if(all_results):
        search_results.update({'pages':list(SEARCH_VECTORS['all_pages'](request).autocomplete(query))})
        search_results.update({'people':list(SEARCH_VECTORS['people'](request).autocomplete(query))})
        search_results.update({'teams':list(SEARCH_VECTORS['teams'](request).autocomplete(query))})
    else:
        search_results.update({'pages':list(SEARCH_VECTORS['all_pages'](request).autocomplete(query)[:limit])})
        search_results.update({'people':list(SEARCH_VECTORS['people'](request).autocomplete(query)[:limit])})
        search_results.update({'teams':list(SEARCH_VECTORS['teams'](request).autocomplete(query)[:limit])})
