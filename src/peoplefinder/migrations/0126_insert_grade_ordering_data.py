# Generated by Django 5.1.8 on 2025-04-23 13:49

from django.db import migrations


def insert_grade_ordering_data(apps, schema_editor):
    Grade = apps.get_model("peoplefinder", "Grade")
    Grade.objects.filter(code="fco_s1").update(ordering=0)
    Grade.objects.filter(code="fco_s2").update(ordering=0)
    Grade.objects.filter(code="fco_s3").update(ordering=0)
    Grade.objects.filter(code="apprentice").update(ordering=0)
    Grade.objects.filter(code="fast_stream").update(ordering=0)
    Grade.objects.filter(code="fast_track").update(ordering=0)
    Grade.objects.filter(code="non_graded_special_advisor").update(ordering=0)
