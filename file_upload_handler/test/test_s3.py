...
from file_upload_handler.s3 import S3FileUploadHandler
from django.test.client import RequestFactory
from unittest.mock import patch, call, MagicMock

from file_upload_handler.signature import verify
from file_upload_handler.s3 import ThreadedS3ChunkUploader
...
class S3FileHandlerTestCase(TestCase):
    ...
    def test_addition_of_av_header(self, storage_file, storage, client):
        ...
        second_copy_obj_call_list = self.s3_file_handler.s3_client.copy_object.call_args_list[1][1]

        assert "Metadata" in second_copy_obj_call_list
        assert "av-passed" in second_copy_obj_call_list["Metadata"]
        assert second_copy_obj_call_list["Metadata"]["av-passed"]
        assert "av-signature" in second_copy_obj_call_list["Metadata"]

        # Check that signature matches
        av_signature = second_copy_obj_call_list["Metadata"]["av-signature"]
        assert verify(e_tag.encode(), av_signature)
...
class ThreadedS3ChunkUploaderTestCase(TestCase):
    ...
    def test_add_future_with_body(self, client):
        ...
        assert threaded_s3_uploader.current_queue_size == 9
        ...
        assert threaded_s3_uploader.current_queue_size == 0
        assert len(threaded_s3_uploader.futures) == 1

        # Wait for upload threads to complete
        concurrent.futures.wait(
            threaded_s3_uploader.futures,
