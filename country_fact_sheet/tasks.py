...
from core.utils import get_s3_file_body, run_anti_virus
    ...
    for file in latest_unprocessed:
        try:
            ...
            if anti_virus_result["malware"]:
                logger.warning(f"Found virus in file {file.s3_document_file.name}")
                file.status = CountryFactSheetUpload.ERROR
                file.user_error_message = "A virus was found in the file"
                file.error_message = str(anti_virus_result)
                file.save()
                continue
            ...
            file.user_error_message = "Could not process file, please contact Live Services for help"
